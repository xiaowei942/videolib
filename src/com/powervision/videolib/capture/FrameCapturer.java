...
public class FrameCapturer {
    public static final int FILE_FORMAT_JPEG = 0;
    public static final int FILE_FORMAT_PNG = 1;

    private static FrameCapturer mInstance;
    ...
    public static synchronized FrameCapturer getInstance() {
        if(mInstance == null) {
            mInstance = new FrameCapturer();
        }
    ...
    FrameCapturer(String name, Bitmap bitmap, int format, int quality) {
        setFilePath(name);
